...
def detection_target_layer(proposals, gt_class_ids, gt_boxes, gt_masks, config):
    ...
    # print(gt_class_ids)
    # print(gt_boxes)
    # print(torch.nonzero(gt_class_ids < 0))
    # Handle COCO crowds
    ...
    if torch.nonzero(positive_roi_bool).size()[0]:
        ...
        masks = Variable(CropAndResizeFunction.apply(roi_masks.unsqueeze(1), boxes, box_ids,config.MASK_SHAPE[0], config.MASK_SHAPE[1], 0), requires_grad=False)
...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, image_index):
        ...
        image, image_metas, gt_class_ids, gt_boxes, gt_masks, gt_tab_struct = \
        ...
        return images, image_metas, rpn_match, rpn_bbox, gt_class_ids, gt_boxes, gt_masks, gt_tab_struct
...
class MaskRCNN(nn.Module):
    ...
    def train_epoch(self, datagenerator, optimizer, steps):
        ...
        for inputs in datagenerator:
            ...
            printProgressBar(step + 1, steps, prefix="\t{}/{}".format(step + 1, steps),
                             suffix="Complete - loss: {}".format(
                                 str(loss.item())), length=10)

            # Statistics

            loss_sum += loss.squeeze(dim=0) / steps
            loss_rpn_class_sum += rpn_class_loss.squeeze(dim=0) / steps
            loss_rpn_bbox_sum += rpn_bbox_loss.squeeze(dim=0) / steps
            loss_mrcnn_class_sum += mrcnn_class_loss.squeeze(dim=0) / steps
            loss_mrcnn_bbox_sum += mrcnn_bbox_loss.squeeze(dim=0) / steps
            loss_mrcnn_mask_sum += mrcnn_mask_loss.squeeze(dim=0) / steps
    ...
    def valid_epoch(self, datagenerator, steps):
        ...
        for inputs in datagenerator:
            ...
            # Progress
            printProgressBar(step + 1, steps, prefix="\t{}/{}".format(step + 1, steps),
                             suffix="Complete - loss: {}".format(str(loss.item())), length=10)
